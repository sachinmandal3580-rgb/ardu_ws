// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from ardupilot_msgs:srv/ModeSwitch.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "ardupilot_msgs/srv/detail/mode_switch__struct.h"
#include "ardupilot_msgs/srv/detail/mode_switch__type_support.h"
#include "ardupilot_msgs/srv/detail/mode_switch__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace ardupilot_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ModeSwitch_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ModeSwitch_Request_type_support_ids_t;

static const _ModeSwitch_Request_type_support_ids_t _ModeSwitch_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ModeSwitch_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ModeSwitch_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ModeSwitch_Request_type_support_symbol_names_t _ModeSwitch_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, ardupilot_msgs, srv, ModeSwitch_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, ardupilot_msgs, srv, ModeSwitch_Request)),
  }
};

typedef struct _ModeSwitch_Request_type_support_data_t
{
  void * data[2];
} _ModeSwitch_Request_type_support_data_t;

static _ModeSwitch_Request_type_support_data_t _ModeSwitch_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ModeSwitch_Request_message_typesupport_map = {
  2,
  "ardupilot_msgs",
  &_ModeSwitch_Request_message_typesupport_ids.typesupport_identifier[0],
  &_ModeSwitch_Request_message_typesupport_symbol_names.symbol_name[0],
  &_ModeSwitch_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ModeSwitch_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ModeSwitch_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &ardupilot_msgs__srv__ModeSwitch_Request__get_type_hash,
  &ardupilot_msgs__srv__ModeSwitch_Request__get_type_description,
  &ardupilot_msgs__srv__ModeSwitch_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace ardupilot_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, ardupilot_msgs, srv, ModeSwitch_Request)() {
  return &::ardupilot_msgs::srv::rosidl_typesupport_c::ModeSwitch_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__struct.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__type_support.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace ardupilot_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ModeSwitch_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ModeSwitch_Response_type_support_ids_t;

static const _ModeSwitch_Response_type_support_ids_t _ModeSwitch_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ModeSwitch_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ModeSwitch_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ModeSwitch_Response_type_support_symbol_names_t _ModeSwitch_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, ardupilot_msgs, srv, ModeSwitch_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, ardupilot_msgs, srv, ModeSwitch_Response)),
  }
};

typedef struct _ModeSwitch_Response_type_support_data_t
{
  void * data[2];
} _ModeSwitch_Response_type_support_data_t;

static _ModeSwitch_Response_type_support_data_t _ModeSwitch_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ModeSwitch_Response_message_typesupport_map = {
  2,
  "ardupilot_msgs",
  &_ModeSwitch_Response_message_typesupport_ids.typesupport_identifier[0],
  &_ModeSwitch_Response_message_typesupport_symbol_names.symbol_name[0],
  &_ModeSwitch_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ModeSwitch_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ModeSwitch_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &ardupilot_msgs__srv__ModeSwitch_Response__get_type_hash,
  &ardupilot_msgs__srv__ModeSwitch_Response__get_type_description,
  &ardupilot_msgs__srv__ModeSwitch_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace ardupilot_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, ardupilot_msgs, srv, ModeSwitch_Response)() {
  return &::ardupilot_msgs::srv::rosidl_typesupport_c::ModeSwitch_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__struct.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__type_support.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace ardupilot_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ModeSwitch_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ModeSwitch_Event_type_support_ids_t;

static const _ModeSwitch_Event_type_support_ids_t _ModeSwitch_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ModeSwitch_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ModeSwitch_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ModeSwitch_Event_type_support_symbol_names_t _ModeSwitch_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, ardupilot_msgs, srv, ModeSwitch_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, ardupilot_msgs, srv, ModeSwitch_Event)),
  }
};

typedef struct _ModeSwitch_Event_type_support_data_t
{
  void * data[2];
} _ModeSwitch_Event_type_support_data_t;

static _ModeSwitch_Event_type_support_data_t _ModeSwitch_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ModeSwitch_Event_message_typesupport_map = {
  2,
  "ardupilot_msgs",
  &_ModeSwitch_Event_message_typesupport_ids.typesupport_identifier[0],
  &_ModeSwitch_Event_message_typesupport_symbol_names.symbol_name[0],
  &_ModeSwitch_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ModeSwitch_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ModeSwitch_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &ardupilot_msgs__srv__ModeSwitch_Event__get_type_hash,
  &ardupilot_msgs__srv__ModeSwitch_Event__get_type_description,
  &ardupilot_msgs__srv__ModeSwitch_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace ardupilot_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, ardupilot_msgs, srv, ModeSwitch_Event)() {
  return &::ardupilot_msgs::srv::rosidl_typesupport_c::ModeSwitch_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "ardupilot_msgs/srv/detail/mode_switch__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace ardupilot_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _ModeSwitch_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ModeSwitch_type_support_ids_t;

static const _ModeSwitch_type_support_ids_t _ModeSwitch_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ModeSwitch_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ModeSwitch_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ModeSwitch_type_support_symbol_names_t _ModeSwitch_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, ardupilot_msgs, srv, ModeSwitch)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, ardupilot_msgs, srv, ModeSwitch)),
  }
};

typedef struct _ModeSwitch_type_support_data_t
{
  void * data[2];
} _ModeSwitch_type_support_data_t;

static _ModeSwitch_type_support_data_t _ModeSwitch_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ModeSwitch_service_typesupport_map = {
  2,
  "ardupilot_msgs",
  &_ModeSwitch_service_typesupport_ids.typesupport_identifier[0],
  &_ModeSwitch_service_typesupport_symbol_names.symbol_name[0],
  &_ModeSwitch_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t ModeSwitch_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ModeSwitch_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &ModeSwitch_Request_message_type_support_handle,
  &ModeSwitch_Response_message_type_support_handle,
  &ModeSwitch_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    ardupilot_msgs,
    srv,
    ModeSwitch
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    ardupilot_msgs,
    srv,
    ModeSwitch
  ),
  &ardupilot_msgs__srv__ModeSwitch__get_type_hash,
  &ardupilot_msgs__srv__ModeSwitch__get_type_description,
  &ardupilot_msgs__srv__ModeSwitch__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace ardupilot_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, ardupilot_msgs, srv, ModeSwitch)() {
  return &::ardupilot_msgs::srv::rosidl_typesupport_c::ModeSwitch_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
